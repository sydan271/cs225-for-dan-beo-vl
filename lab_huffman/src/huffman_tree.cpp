...
HuffmanTree::TreeNode *HuffmanTree::removeSmallest(queue<TreeNode *> &singleQueue,
   ...
   s = singleQueue.front();
   m = mergeQueue.front();
   auto sm = s->freq.getFrequency();
   auto fm = m->freq.getFrequency();
...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies) {
   ...
   for (const Frequency &frq : frequencies) {
      TreeNode *node = new TreeNode(frq);
   ...
   while (singleQueue.size() + mergeQueue.size() >= 2) {
      ...
      TreeNode *newNode = new TreeNode(l->freq.getFrequency() + r->freq.getFrequency());
      newNode->left = l;
      newNode->right = r;
...
void HuffmanTree::writeTree(BinaryFileWriter &bfile) { writeTree(root_, bfile); }
...
public:
   ...
   HuffmanTreeNodeDescriptor left() const { return HuffmanTreeNodeDescriptor(subRoot_->left); }
   HuffmanTreeNodeDescriptor right() const { return HuffmanTreeNodeDescriptor(subRoot_->right); }
...
Frequency HuffmanTree::testRemoveSmallest(vector<Frequency> single, vector<Frequency> merge) {
