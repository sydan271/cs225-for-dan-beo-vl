...
void List<T>::waterfall() {
   ...
   while (curr && curr != tail_) {
      ListNode *prevCurr = curr->prev;

      prevCurr->next = curr->next;
      prevCurr->next->prev = prevCurr;
...
void List<T>::reverse(ListNode *&startPoint, ListNode *&endPoint) {
   ...
   if (!startPoint->prev) head_ = startPoint;
   if (!endPoint->next) tail_ = endPoint;
}
...
typename List<T>::ListNode *List<T>::mergesort(ListNode *start, int chainLength) {
   ...
   ListNode *secondChain = split(start, first_len);
   return merge(mergesort(start, first_len), mergesort(secondChain, chainLength - first_len));
}
