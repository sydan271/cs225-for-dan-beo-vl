...
TEST_CASE("KDTree::findNearestNeighbor (2D), returns correct result", "[weight=1][part=1]") {
   ...
   compareBinaryFiles(fname,
                      "../data/kdtree_" + to_string(K) + "_" + to_string(size) + "-expected.kd");
...
*/
TEST_CASE("KDTree::findNearestNeighbor (2D), testing correct path with fence jumping",
          "[weight=1][part=1]") {
...
TEST_CASE("KDTree::findNearestNeighbor (3D), testing that left recursion does not include the root",
          "[weight=1][part=1]") {
   double coords[10][3] = {{5, 3, 1},    {3, 1, 10},   {2, 0, 8},    {4, 3, 0},    {0, 2, 9},
