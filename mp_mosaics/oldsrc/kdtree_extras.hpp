...
template <int Dim>
void KDTree<Dim>::printTree(ostream &out /*cout*/, colored_out::enable_t enable_bold /* = COUT */,
...
template <int Dim> int KDTree<Dim>::getPrintData(KDTreeNode *subroot) const {
   using std::max;
   if (subroot == NULL)
      return -1;
...
template <int Dim>
void KDTree<Dim>::printTree(KDTreeNode *subroot, vector<string> &output, int left, int top, int width,
                            int currd) const {
   ...
   // Output data
   for (int dim = 0; dim < Dim; dim++)
      for (size_t i = 0; i < nodeStr[dim].length() && (left + width / 2 + i < output[top + dim].length()); i++) // TODO
   ...
   if (subroot->left != NULL) { // node->left  != NULL
      ...
      // draw left subtree
      printTree(subroot->left, output, left, top + Dim + 1, width / 2 - 1, (currd + 1) % Dim);
   }
   // Print right child
   if (subroot->right) { // node->right != NULL
      ...
      for (int pos = branch_pos + 1; pos < rightcenter; pos++)
         output[top + Dim][pos] = '_';
      // draw right subtree
      printTree(subroot->right, output, left + width / 2 + 2, top + Dim + 1, width / 2 - 1, (currd + 1) % Dim);
   }
}
