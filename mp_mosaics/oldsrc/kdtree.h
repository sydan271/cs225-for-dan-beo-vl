...
private:
   ...
   /** Helper function for grading */
   void printTree(KDTreeNode *subroot,
   ...
   void
   makeTree(KDTreeNode *&subroot, std::vector<Point<Dim>> &Points, int left, int right, int curDim);

   // clean the tree
   void clear(KDTreeNode *&subroot);

   // copy from other recursively
   void copyTree(KDTreeNode *&subroot, const KDTreeNode *otherSubroot);

   void findNearestNeighborRecursive(KDTreeNode *subroot,
                                     const Point<Dim> &query,
                                     Point<Dim> &curBest,
                                     int curDim) const;

   double calDis(const Point<Dim> &target, const Point<Dim> &point) const;
};
...
// Partition the list around the pivot as in the quickselect algorithm
template <typename RandIter, typename Comparator>
RandIter partitionAtPivot(RandIter begin, RandIter end, RandIter pivot, Comparator cmp);
