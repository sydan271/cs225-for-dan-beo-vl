...
private:
   ...
   /** Helper function for grading */
   void printTree(
       KDTreeNode *subroot, std::vector<std::string> &output, int left, int top, int width, int currd) const;

   /**
    * @todo Add your helper functions here.
    */
   void makeTreeRecursively(
   ...
   Point<Dim> findNearestNeighborRecursive(KDTreeNode *subroot,
                                           int dimension,
                                           Point<Dim> currentBest,
                                           const Point<Dim> &query) const;

   double calulateDis(const Point<Dim> &point, const Point<Dim> &query) const;

   void
   findNearestVer2(KDTreeNode *subroot, int dimension, Point<Dim> &curBest, const Point<Dim> &query) const;
};
