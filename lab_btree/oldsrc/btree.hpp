...
void BTree<K, V>::insert(BTreeNode *subroot, const DataPair &pair) {
   ...
   std::vector<DataPair> &elements = subroot->elements;
   if (first_larger_idx >= elements.size() || !(elements[first_larger_idx] == pair)) {
      if (subroot->is_leaf) {
         elements.insert(elements.begin() + first_larger_idx, pair);
      } else {
