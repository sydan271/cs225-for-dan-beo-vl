 ...
 */
template <class K, class V> class BST {
private:
  /**
...
private:
  ...
  void getInorderTraversal(const Node *subRoot,
                           std::vector<K> &traversal) const;

  /**
   * Gets the pre-order traversal of an BST tree's keys.
   * @param subRoot The current node in the recursion
   */
  void getPreorderTraversal(const Node *subRoot,
                            std::vector<K> &traversal) const;
  ...
  // my own helper
  Node*& findIOP(Node*& subtree);
};

/**
 * Builds a BST by inserting all elements in the list
 * @param std::vector<std::pair<K, V>> The input list consisting of (string,
 * int) pairs
 * @return The BST containing all unique inputs (ignoring duplicates once
 * inserted once)
 */
...
/**
 * Builds all possible permutations of an input list (treating all items as
 * unique)
 * @param std::vector<std::pair<K, V>> The input list consisting of (string,
 * int) pairs
