...
bool PNG::operator==(PNG const &other) const {
   ...
   for (unsigned i = 0; i < width_ * height_; i++) {
      ...
      if (rgbaSpace1.r != rgbaSpace2.r || rgbaSpace1.g != rgbaSpace2.g ||
          rgbaSpace1.b != rgbaSpace2.b || rgbaSpace1.a != rgbaSpace2.a) {
         return false;
      }
   }
...
std::ostream &operator<<(std::ostream &os, PNG const &png) {
   ...
   os << "PNG(w=" << png.width() << ", h=" << png.height() << ", hash=" << std::hex << hash
      << std::dec << ")";
   return os;
}
