...
private:
   ...
   // helper to get the idx-th tile number
   uint64_t getTile(unsigned idx) const;

   // helper to get a map from values (0,1,2,..,15) to position index(0,...15)
   void getMap(std::map<char, unsigned> &) const;
};
...
solveBFS(const PuzzleState &startState, const PuzzleState &desiredState, size_t *iterations = NULL);
