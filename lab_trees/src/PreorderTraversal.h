 ...
 */
template <typename T> class PreorderTraversal : public TreeTraversal<T> {
public:
   /**
    ...
    */
   typename TreeTraversal<T>::Iterator begin() { return typename TreeTraversal<T>::Iterator(this, root); }
    ...
    */
   typename TreeTraversal<T>::Iterator end() { return typename TreeTraversal<T>::Iterator(); }
    ...
    * @return The current node in the traversal
    */
   typename BinaryTree<T>::Node *peek() const { return empty() ? NULL : stack.top(); }
