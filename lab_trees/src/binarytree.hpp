 ...
 * @param subRoot
 */
template <typename T> void BinaryTree<T>::printLeftToRight(const Node *subRoot) const {
   // Base case - null node
   if (subRoot == NULL)
      return;
...
template <typename T> bool BinaryTree<T>::isOrderedRecursive(const Node *subRoot, const T *&prev) const {
   if (!subRoot)
      return true;
   ...
   if (prev && *prev > subRoot->elem)
      return false;
   prev = &(subRoot->elem);

   if (!isOrderedRecursive(subRoot->right, prev))
      return false;
   return true;
}
